...
public class Passenger : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (distToReaderOut > 2 && dist > radius && bus.Rejsekort)
        {
            var cubeRenderer = agent.GetComponent<Renderer>();
            cubeRenderer.material.SetColor("_Color", Color.white);
            BT.trackBeacon = false;
        ...
        if (dist > radius && bus.Rejsekort == false && bus.startDriving == false)
        {
            var cubeRenderer = agent.GetComponent<Renderer>();
            cubeRenderer.material.SetColor("_Color", Color.white);
            BT.trackBeacon = false;
        ...
        if (dist < radius && bus.Rejsekort == false && enterBus == false)
        {
            enterBus = true;
            EnterBus();
            BT.trackBeacon = true;
        ...
        if (BT.trackBeacon)
        {
            BT.currentDistance = dist;
            print("Strength:" + dist);
            if ((((radius - dist) / radius) * 100) > 60)
            {
                var cubeRenderer = agent.GetComponent<Renderer>();
        ...
        if (dist < radius && bus.Rejsekort == false && goToExit) 
        {
        ...
        if (dist < radius && bus.Rejsekort) //Start tracking the beacon time
        {
            BT.trackBeacon = true;
        } 
